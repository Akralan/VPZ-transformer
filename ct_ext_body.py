...
def extract_images_from_json(json_path, out_dir):
    ...
    # Collecter tous les blocs d'images
    images = []
            ...
            if "meta_infos" in data:
                ...
                if meta == "::fwData::Image":
                    ...
                    if blob:
                        ...
                        buffer_size = int(blob.get("buffer_size", 0))
                        dtype = (
                            attrs.get("array", {})
                            .get("object", {})
                            .get("attributes", {})
                        ...
                        print(f"\n🩻 Found {buffer_file} (size: {buffer_size} bytes, {dtype}, {dims})")
    ...
    # Exporter uniquement le plus gros fichier
    if not images:
        print("⚠️ Aucune image trouvée")
        return
    
    largest = max(images, key=lambda x: x['buffer_size'])
    print(f"\n✅ Sélection du fichier le plus volumineux: {largest['buffer_file']} ({largest['buffer_size']} bytes)")
    
    buffer_file_normalized = largest['buffer_file'].replace("\/", os.sep).replace("/", os.sep)
    ...
    out_name = os.path.splitext(os.path.basename(largest['buffer_file']))[0] + ".nii.gz"
    vol = load_raw_volume(abs_path, largest['dims'], np.dtype(largest['dtype']), largest['strides'])
